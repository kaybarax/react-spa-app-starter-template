...
describe('util', () => {
  ...
  describe('objectAHasSameKeysAsObjectB', () => {
    it('returns true when both objects have identical keys', () => {
      expect(objectAHasSameKeysAsObjectB({ a: 1, b: 2 }, { a: 3, b: 4 })).toBe(true);
    });

    it('returns false when objB is missing a key that objA has', () => {
      expect(objectAHasSameKeysAsObjectB({ a: 1, b: 2 }, { a: 1 })).toBe(false);
    });

    it('returns false when objects have same keys but different count', () => {
